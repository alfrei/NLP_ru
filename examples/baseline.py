...
tmp = []
for d in corpus:
    # d = remove_word(d, "ht")  # remove hashtags
...
def fit_model(X_train,X_test,y_train,y_test,model):
    ...
    print('acc:%s' % metrics.accuracy_score(y_test, yhat))
...
yhat = []
for model in [lr, xgb, nb, rf]:
    yhat.append(fit_model(X_train,X_test,y_train,y_test,model))


# simple blending with weights
def combine_yhat(yhat):

    y_predict = []
    for pred, pred_proba in yhat:
        if pred_proba is None:
            ...
            for p in pred:
                ...
                p_ohe[p] = 1
                p_out.append(p_ohe)
            y_predict.append(p_out)
        else:
            y_predict.append(pred_proba)
            
    y_predict = np.array(y_predict)
    return y_predict

yhat = combine_yhat(yhat)
weights = np.array([10, 3, 0.1, 5]).reshape(1,4)
y_weighted = np.tensordot(weights, yhat, [1,0])[0]
...
print("-"*60)
print("simple ensemble blending")
print(metrics.accuracy_score(y_test, np.apply_along_axis(arr=y_weighted, axis=1, func1d=max_index)))
print(metrics.confusion_matrix(y_test, np.apply_along_axis(arr=y_weighted, axis=1, func1d=max_index)))
